...
void command_get(struct packet* shp, struct packet* data, int sfd_client)
{
	...
	shp->type = INFO;
	shp->comid = GET;
	strcpy(shp->buffer, f ? "File trovato; elaboro" : "Errore in apertura del file.");
	//printpacket(shp, HP);
	data = htonp(shp);
	if((x = send(sfd_client, data, size_packet, 0)) != size_packet)
		er("send()", x);
...
void command_put(struct packet* shp, struct packet* data, int sfd_client)
{
	...
	shp->type = INFO;
	shp->comid = PUT;
	strcpy(shp->buffer, f ? "Tutto in ordine; elaboro" : "Errore in apertura del file per la scrittura lato server.");
	//printpacket(shp, HP);
	data = htonp(shp);
	if((x = send(sfd_client, data, size_packet, 0)) != size_packet)
		er("send()", x);
...
void command_mkdir(struct packet* shp, struct packet* data, int sfd_client)
{
	...
	if(d)
	{	
		strcpy(message, "gia' esistente");
		closedir(d);
	}
	else if(mkdir(shp->buffer, 0777) == -1)
	{
		fprintf(stderr, "Percorso errato.\n");
		strcpy(message, "fallito");
	}
	else
		strcpy(message, "completato");
	int x;
	shp->type = INFO;
	strcpy(shp->buffer, message);
	data = htonp(shp);
